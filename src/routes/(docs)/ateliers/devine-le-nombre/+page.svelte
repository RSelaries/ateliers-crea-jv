<script>
    import Link from "$lib/components/Link.svelte";
</script>


<svelte:head>
    <title>Atelier - Devine le nombre</title>
</svelte:head>


<h1 id="devine-le-nombre">Devine le nombre</h1>
<p>
    Dans cet atelier, nous allons découvrir l'interface de Godot et créer notre premier jeu.
</p>
<details>
    <summary>Concepts abordés</summary>
    <p>Jsp encore</p>
</details>

<h2 id="jeu">Jeu</h2>
<p>
    <i>*iframe du jeu*</i>
</p>

<h2 id="creation-du-projet">Création du projet</h2>
<p>
    Pour cet atelier et tous les suivant, il faut installer Godot.
    Vous pouvez suivre <b>étape-par-étape</b> comment faire <Link href="godot/installation-et-configuration">ici</Link>.
</p>